...
var calendar = function(id){
	...
	getid(id).onclick = function(ev){
		...
		if(lowerCase(target,"a")){
			target.name == "prevM"?(month>0?month--:(month=11,year--)):target.name == "prevY"?(year--):target.name =="nextY"?(year++):(month<11?month++:(month=0,year++));
...
calendar.prototype.builds = function(date){
	...
	getid("year").innerHTML = year+"年";
	getid("month").innerHTML = month+"月";
	getid("day").innerHTML = now+"日";
